...
public class TabelProductController implements Initializable {
    ...
    /*
     public void schearchBox(){ //Поиск в таблице
        FilteredList<Product> filteredList = new FilteredList<>(data2, b -> true);
        schearchBox.textProperty().addListener((observable, oldValue , newValue) -> {
            filteredList.setPredicate(Product -> {
                if (newValue.isEmpty() || newValue.isBlank() || newValue == null) {
                    return true;
                }
                String schearchKeyword = newValue.toLowerCase();

                if (Product.getTitle().toLowerCase().indexOf(schearchKeyword) > -1 ) {
                    return true;
                } else
                    return false;
            });
        ...
        sortedList.comparatorProperty().bind(TableStock.comparatorProperty());
        TableStock.setItems(sortedList);
    }
     */
