...
public class DataBaseHandlerOrder extends Config{
    ...
    public void editOrders(String newId, Integer newId_customer, Integer newId_worker, String newStatus){
        String Update = "UPDATE storage.order SET id_customer=" + "'" + newId_customer + "'" +
                ",id_worker=" + "'" + newId_worker + "'" + ",status=" +"'"+ newStatus +"'"+
                " where id_order=" + newId;
        try {
            preparedStatement = getDbConnection().prepareStatement(Update);
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException("ads",e);
        }
    }
